...
- `proxy_intercept_errors on` combined with `error_page 500 501 502 503 504 @50x` means upstream 5xx bodies (including ASBS's structured `ErrorResponse` with its `correlationId`) are replaced by the fixed JSON above; the status is rewritten to `500` even for a `503`. [VERIFIED] `:37-38,131-134`.
